...
const id = url.searchParams.get("device_edit");
...
const rules = {
    name_th: { required: helpers.withMessage('กรุณาระบุชื่อของ Device', required) },
    name_en: { },
    // kafka: { required: helpers.withMessage('กรุณรระบุ Kafka Topic', required) },
    device_status: { required: helpers.withMessage('กรุณรระบุ Status', required) },
    image: { }
...
const submit = async () => {
    ...
    if (id) {
        if (result) {
            ApiMain.put("/devices/" + id, {
                name: {
                    th: dataform.name_th,
                    en: dataform.name_en,
                },
                // kafkaTopic: dataform.kafka,
...
const upload_image = (event) => {
  ...
  ApiMain.post("/image/upload?folder=devices",formdata).then((data) => {
      if (data.status == 201) {
            ...
            <form @submit.prevent="submit">
               ...
               <div class="block md:flex w-full gap-2">
                <div class="mb-4 w-full">
                    <label for="name_th"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ชื่อ Device<span class="text-red-500">*</span></label>
                    <input type="text" id="name_th" v-model="dataform.name_th"
                        :class="v$.name_th.$error ? 'border-red-300' : 'border-gray-300'" placeholder="กรุณาระบุชื่อ Device"
                ...
                <div class="mb-4 w-full">
                    <label for="name_en" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Device Name</label>
                    <input type="text" id="name_en" v-model="dataform.name_en" 
                        :class="v$.name_en.$error ? 'border-red-300' : 'border-gray-300'" placeholder="Please specify Device name"
                        class="shadow-sm bg-gray-50 border  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light">
               ...
               <!-- <div class="mb-6 w-full">
                    <label for="kafka"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kafka Topic</label>
                    <input type="text" id="kafka" v-model="dataform.kafka"
                        :class="v$.kafka.$error ? 'border-red-300' : 'border-gray-300'"
                        class="shadow-sm bg-gray-50 border  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light">
                    <small v-if="v$.kafka.$error" class="text-red-500 float-right">{{ v$.kafka.$errors[0].$message
                    }}</small>
                </div> -->
