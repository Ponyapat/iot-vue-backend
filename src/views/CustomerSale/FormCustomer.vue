...
const upload_image = async (event)=>{
  ...
  if(len_over.value == true){
    input_file.classList.add('bg-[#FDF2F2]');
  }
  else {
    input_file.classList.add('bg-[#F3FAF7]');
  }
  spiner_load.value = false ;
          ...
          <div class="flex flex-row justify-center mb-4">
            <div class="w-full md:w-[800px] h-full">
              ...
              <div>
                ...
                <!-- Upload File image -->
                <div>
                  <label for="uoload" class="block mb-2 text-base font-medium text-black dark:text-white" >อัปโหลดรูปภาพ</label>
                  <label for="dropzone-file" id="file_input" class="flex flex-col items-center justify-center w-full h-[150px] border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                      ...
                      <span class="text-md font-bold">กำลังอัปโหลด</span>
                    </div>
                    <div v-else class="flex flex-col items-center justify-center pt-5 pb-6">
                          <svg aria-hidden="true" class="w-10 h-10 mb-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                          <p class="mb-2 text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold">คลิกเพื่ออัปโหลดรูปภาพ </span></p>
                          <p class="text-xs text-gray-500 dark:text-gray-400">SVG, PNG, JPG , JPEG </p>
                      </div>
                      <input id="dropzone-file"  @change="upload_image" type="file" accept=".png ,.jpg ,.jpeg,.svg" class="hidden"  multiple/>
                  </label>
                  <div v-if="id" class="flex flex-row items-center gap-2 mt-4">
                ...
                <div v-else class="flex flex-row items-center gap-2  mt-4">
                  <div v-for="(item,index) in dataform.otherCustomerImage" :key="index" class="text-center relative">
