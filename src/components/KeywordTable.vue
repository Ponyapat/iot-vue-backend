...
const pageNext = (page) => {
  ...
  ApiMain.get("/market/keyword?order=ASC&page=" + (page + 1) + "&take=" + perPage.value)
  .then((data) => {
    states.keyword = data.data.data;
  });
};
const nextPage = () => {
  ...
  if (x < pagelist.slice(-1)[0]) {
    ApiMain.get("/market/keyword?order=ASC&page=" + (x + 1) + "&take=" + perPage.value)
      .then((data) => {
        states.keyword = data.data.data;
      });
  }
...
const previousPage = () => {
  let x = currentPage.value--;
  if (x > 0) {
    ApiMain.get("/market/keyword?order=ASC&page=" + (x + 1) + "&take=" + perPage.value).then((data) => {
      states.keyword = data.data.data;
    });
  }
  else {
...
const lastPage = () => {
  let pagelist = pagesList.value;
  currentPage.value = pagelist.slice(-1)[0];
  ApiMain.get("/market/keyword?order=ASC&page=" + (currentPage.value+1)  + "&take=" + perPage.value).then((data) => {
      states.keyword = data.data.data;
    });
};
const firstPage = () => {
...
const fetchRoleData = () => {
  ApiMain.get("/market/keyword?order=ASC&page=" + (currentPage.value + 1) + "&take=" + perPage.value)
    .then((response) => {
      states.keyword = response.data.data;
      items.value = response.data.meta.itemCount;
    });
};

const edit = (id) => {
  router.push("/keyword/edit/" + id);
};
...
const del = (item) => {
  ...
  Swal.fire({
    title: "ยืนยันการลบ",
    text: `คุณต้องการลบค้นหา ${item.keyword} ใช่หรือไม่`,
    icon: "warning",
...
onBeforeMount(() => {
  if(ck_pms("update","keyword")){
    per_edit.value = true
  }
  if(ck_pms("delete","keyword")){
    per_del.value = true
  }
  fetchRoleData();
});
...
<template>
  <div>
    <table>
      ...
      <tbody>
        <tr
          v-for="(item, index) in states.keyword"
          :key="index"
          :class="[tableTrStyle, index % 2 === 0 ? tableTrOddStyle : '']"
        >
          <td class="p-3" data-label="ID">{{ item.id }}</td>
          <td class="p-3" data-label="Name">{{ item.keyword }}</td>
