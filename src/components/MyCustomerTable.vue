...
<template>
  <div class="flex flex-row">
    <div class="w-1/4 ml-4 mr-[40px] mt-10">
      ...
      <small class="opacity-60">ค้นหาด้วย ชื่อลูกค้า ,อีเมล, เบอร์โทรศัพท์มือถือ </small>
    </div>
    <div class="mt-5">
      <label for="status" class="block text-sm font-medium text-gray-900 dark:text-white opacity-70">สถานะ</label>
      <select id="status" @change="statusFilter($event)"
        ...
        <option value="1">ยืนยันตัวตนแล้ว</option>
        <option value="0">ยังไม่ได้ยืนยันตัวตน</option>
      </select>
    </div>
  </div>
  <div class="relative mt-4">
    ...
    <table class="w-full text-sm text-left text-black dark:text-white">
      ...
      <tbody>
        <tr v-for="(item, index) in customers_list.slice(pageStart, pageStart + perPage)" :key="item.id"
          ...
          <td class="text-center w-[200px]">
            <div v-if="item.activated == 0" class="text-orange-500 rounded-2xl py-1"><i class="fa-solid fa-exclamation bg-orange-200 px-2.5 py-1 rounded-full mr-1"></i>ยังไม่ได้ยืนยันตัวตน</div>
            <div v-if="item.activated == 1" class="text-green-500 rounded-2xl py-1"><i class="fa-solid fa-check text-green-700 bg-green-200 p-1 rounded-full mr-1"></i>ยืนยันตัวตนแล้ว</div>
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <nav :class="lightBorderStyle" aria-label="Page navigation example" class="py-2 mx-3 lg:mx-6 border-t border-b ">
    <ul class="inline-flex items-center -space-x-px">
      ...
      <li v-for="item in totalPage" :key="item" :class="{ active: currPage == item }" @click="setPage(item)"
